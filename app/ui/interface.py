...
class BjorgsunUI:
    def __init__(self, dev_mode: bool | None = None):
        ...
        try:
            # Explicitly start non-topmost; user can enable via toggle.
        ...
        # Neon button helper (accented capsule)
        def _make_neon_button(parent, text, command, color="#31d6ff", padx=14, pady=6):
            return tk.Button(
                parent,
                text=text,
                command=command,
        ...
        self._make_neon_button = _make_neon_button
        self.root.title(
        ...
        self._awake = False
        self._booting = False
        # Manual wake only if explicitly requested via env; default is auto-allowed.
        self._require_wake = _env_flag("UI_REQUIRE_WAKE", "0")
        ...
        try:
            # Let both columns share space; widgets inside handle min sizes.
            self.content.grid_columnconfigure(0, weight=1)
            self.content.grid_columnconfigure(1, weight=1)
            self.content.grid_rowconfigure(0, weight=0)  # wake/orb row
            self.content.grid_rowconfigure(1, weight=1)  # main panes
        except Exception:
            pass

        # Wake bar sits on the left column only
        self.wake_row = tk.Frame(self.content, bg=theme.COLORS["bg"])
        self.wake_row.grid(row=0, column=0, sticky="ew", padx=(12, 6), pady=(0, 6))

        self.left = tk.Frame(self.content, bg=theme.COLORS["bg"])
        self.left.grid(row=1, column=0, sticky="nsew", padx=(12, 6), pady=(0, 12))
        try:
            self.left.grid_rowconfigure(0, weight=0)  # (reserved, e.g., future toggles)
            self.left.grid_rowconfigure(1, weight=1)  # chat/log stack
            self.left.grid_rowconfigure(2, weight=0)  # input bar
            self.left.grid_columnconfigure(0, weight=1)
        except Exception:
            pass

        # Right side: container (orb + mood/diag + system log + hush)
        self.right_container = tk.Frame(self.content, bg=theme.COLORS["bg"])
        # Allow natural expansion; avoid fixed widths to reduce clipping
        self.right_container.grid_propagate(True)
        ...
        try:
            rpw_inner = max(260, rpw - 28)
        except Exception:
            rpw_inner = 280
        # Rows: 0 orb, 1 mood, 2 diagnostics, 3 system log (stretch), 4 hush
        self.right_container.grid_rowconfigure(0, weight=0)  # orb
        self.right_container.grid_rowconfigure(1, weight=0)  # mood
        self.right_container.grid_rowconfigure(2, weight=0)  # diag
        self.right_container.grid_rowconfigure(3, weight=2)  # system log grows
        self.right_container.grid_rowconfigure(4, weight=0)  # hush
        self.right_container.grid_columnconfigure(0, weight=1)
        self.right = tk.Frame(
            self.right_container, bg=theme.COLORS["bg"], width=rpw_inner
        )
        self.right.grid(row=2, column=0, sticky="nsew")
        self.hush_bar = tk.Frame(self.right_container, bg=theme.COLORS["bg"])
        self.hush_bar.grid(row=4, column=0, sticky="ew", padx=(8, 12), pady=(6, 10))
        hush_img = None  # force canvas button to avoid oversized image
        try:
        ...
        # Left stack holds rails + chat modules
        self.left_stack = tk.Frame(self.left, bg=theme.COLORS["bg"])
        self.left_stack.grid(row=1, column=0, sticky="nsew")
        try:
        ...
        # HUD ring - organic, circular center piece (thinking/hearing) on the right column
        try:
            hud_h = int(112 * getattr(self, "_ui_scale", 1.0))
        except Exception:
            hud_h = 112
        self.orb_shell = tk.Frame(self.right_container, bg=theme.COLORS["bg"])
        self.orb_shell.grid(row=0, column=0, sticky="ew", padx=(8, 12), pady=(0, 6))
        self.orb_shell.grid_columnconfigure(0, weight=1)
        ...
        try:
            # Add a subtle HUD frame around the orb
            def _orb_frame(_evt=None):
                try:
                    self.hud_canvas.delete("orb-frame")
                    w = int(self.hud_canvas.winfo_width() or 180)
                    h = int(self.hud_canvas.winfo_height() or hud_h)
                    theme.round_rect(
                        self.hud_canvas,
                        2,
                        2,
            ...
            self.hud_canvas.bind("<Configure>", _orb_frame)
        except Exception:
            pass
        ...
        # Wake strip - hero row with big wake + quick actions
        self._wake_strip = tk.Frame(self.wake_row, bg=theme.COLORS["bg"])
        self._wake_strip.grid(row=0, column=0, sticky="ew")
        wake_shell = tk.Canvas(
            self._wake_strip, height=80, bg=theme.COLORS["bg"], highlightthickness=0
        )
        wake_shell.pack(fill="x", expand=True)

        def _layout_wake(_evt=None):
            try:
                wake_shell.delete("hud")
                w = max(480, int(wake_shell.winfo_width() or 520))
                h = int(wake_shell.winfo_height() or 80)
                # Outer and inner rails for a sci-fi module look
                theme.round_rect(
                    wake_shell,
                    2,
                    2,
                ...
                # Accent bar
                wake_shell.create_rectangle(
                    14,
                    h - 14,
                    min(w - 14, 260),
                    h - 12,
                    fill=theme.COLORS["accent_glow"],
                    width=0,
                    tags=("hud",),
                )
                if not hasattr(wake_shell, "_wake_frame"):
                    inner = tk.Frame(
                        wake_shell, bg="#0d141c", highlightthickness=0, bd=0
                    )
                    wake_shell._wake_frame = inner
                    wake_shell._wake_win = wake_shell.create_window(
                        18, h // 2, anchor="w", window=inner
                    )
                    self._wake_button = self._make_neon_button(
                        inner,
                        "WAKE SYSTEMS",
                        lambda: (
                            setattr(self, "_wake_requested", True),
                            self._do_wake(),
                        ),
                    ...
                    self._wake_button.pack(side="left", padx=(6, 10))
                    self._quick_diag_btn = self._make_neon_button(
                        inner,
                        "SELF-CHECK",
                        self._run_selfcheck,
                    ...
                    self._quick_diag_btn.pack(side="left", padx=(0, 8))
                    self._quick_logs_btn = self._make_neon_button(
                        inner,
                        "OPEN LOGS",
                        self._open_logs,
                        color="#55ff88",
                    ...
                    self._quick_logs_btn.pack(side="left", padx=(0, 8))
                wake_shell.coords(wake_shell._wake_win, 20, h // 2)
                wake_shell.itemconfig(
                    wake_shell._wake_win, width=max(160, w - 40)
                )
            except Exception:
                pass

        wake_shell.bind("<Configure>", _layout_wake)

        # Chat + System Log layout: two-column module grid
        chat_container = tk.Frame(self.left_stack, bg=theme.COLORS["bg"])
        self._stack_left_widget(chat_container, pady=(6, 2), sticky="nsew", weight=1)
        chat_container.grid_columnconfigure(0, weight=1)
        chat_container.grid_rowconfigure(0, weight=1)

        def _make_panel(parent, title: str):
            # HUD-inspired module shell for chat/log panels
            frame = tk.Frame(parent, bg=theme.COLORS["bg"])
            header = tk.Frame(frame, bg=theme.COLORS["bg"])
            header.pack(fill="x", padx=(4, 4), pady=(0, 2))
            tk.Label(
                header,
                text=title.upper(),
            ...
            header_bar = tk.Canvas(
                header, height=3, bg=theme.COLORS["bg"], highlightthickness=0
            )
            header_bar.create_line(
                0, 2, 110, 2, fill=theme.COLORS["accent"], width=3
            )
            header_bar.create_line(
                114, 2, 160, 2, fill=theme.COLORS["accent2"], width=2
            )
            header_bar.pack(side="left", fill="x", expand=True, padx=(0, 6))

            canvas = tk.Canvas(
                frame, bg=theme.COLORS["bg"], highlightthickness=0, height=160
            )
            ...
            def _layout(_evt=None):
                try:
                    ...
                    # Accent tick marks
                    canvas.create_line(
                        18, h - 20, min(w - 18, 180), h - 20, fill="#15d6ff", width=2
                    )
                    canvas.create_line(
                        w - 50,
                        h - 24,
                    ...
                    if not hasattr(canvas, "_text_window"):
                        canvas._text_window = canvas.create_window(
                            18, 14, window=txt, anchor="nw"
                        )
                    canvas.coords(canvas._text_window, 18, 14)
                    canvas.itemconfig(
                        canvas._text_window,
                        width=max(60, w - 36),
                        height=max(80, h - 28),
        ...
        chat_frame, self.chat_console = _make_panel(chat_container, "Chat")
        chat_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 0), pady=(0, 4))

        # Maintain legacy attribute name for downstream code paths
        self.console = self.chat_console
        self._update_wake_strip()
        self._ensure_input_bar(parent=self.left, row=2, columnspan=1)
        try:
            self.entry.bind("<Return>", self._on_enter)
        except Exception:
            pass

    def _ensure_input_bar(self, parent=None, row=None, columnspan=1):
        if getattr(self, "input_bar", None) and self.input_bar.winfo_exists():
            return
        target_parent = parent if parent is not None else self.left
        target_row = row if row is not None else 2
        # Input bar pinned to bottom and stylized
        self.input_bar = tk.Frame(target_parent, bg=theme.COLORS["bg"])
        try:
            self.input_bar.grid(row=target_row, column=0, columnspan=columnspan, sticky="ew", pady=(6, 10), padx=(6, 6))
            self.input_bar.grid_columnconfigure(0, weight=1)
            self.input_bar.grid_columnconfigure(1, weight=0)
        except Exception:
            self.input_bar.pack(fill="x", pady=(6, 10), padx=(6, 6))
        self.input_canvas = tk.Canvas(
            self.input_bar, height=32, bg=theme.COLORS["bg"], highlightthickness=0
        )
        self.entry = tk.Entry(
            self.input_bar,
            fg="#ffffff",
        ...
        self._input_window = self.input_canvas.create_window(
            12, 16, window=self.entry, anchor="w"
        )
        ...
        def _redraw_input(_evt=None):
            try:
                ...
                h = int(self.input_canvas.winfo_height() or 32)
                theme.round_rect(
                    self.input_canvas,
                    2,
                    2,
    ...
    def _request_wake(self):
        ...
        # Mood panel + status + right-side tabs (Modules / Settings)
        self._build_mood_panel()
        self._build_status_panel_main()
        self._build_system_log_panel_main()
        self._build_tabs()
    ...
    def safe_log(self, text, color="#cccccc"):
        ...
        try:
            target = getattr(self, "system_console", None) or getattr(self, "console", None) or getattr(self, "chat_console", None)
            if target:
                self.root.after(0, lambda: self._append_text(target, text, color))
        except Exception:
            pass
        try:
            if self._dev_panel:
    ...
    def _append_text(self, widget: tk.Text, text: str, color: str):
        try:
            ...
            # Guard against early callbacks before consoles are built or transient Tk errors.
            return
    ...
    def _build_tabs(self):
        ...
        actions = tk.Frame(nb, bg=theme.COLORS["bg"])
        commands = tk.Frame(nb, bg=theme.COLORS["bg"])
        nb.add(actions, text="Actions")
    ...
    def _build_quick_actions(self, parent):
        ...
        btn(col, "Auto Titanfall Start", self._auto_start_titanfall)
    ...
    def _build_commands_tab(self, parent):
        ...
        canvas.grid(row=0, column=0, sticky="nsew")
        vbar.grid(row=0, column=1, sticky="ns")
        body = tk.Frame(canvas, bg=theme.COLORS["bg"], padx=10, pady=10)
        content_id = canvas.create_window((10, 10), window=body, anchor="nw")
        ...
        tk.Label(
            body,
            text="Run common slash-style commands without typing.",
            fg=theme.COLORS["muted"],
            bg=theme.COLORS["bg"],
            font=(theme.get_font_family(), 10),
        ).pack(anchor="w", pady=(0, 6))

        grid = tk.Frame(body, bg=theme.COLORS["bg"])
        grid.pack(fill="x", pady=(4, 8))
        grid.columnconfigure((0, 1), weight=1, uniform="cmds")

        def add_cmd(row: int, col: int, label: str, action):
            btn = self._pack_full_button(grid, label, action)
            btn.grid(row=row, column=col, sticky="ew", padx=4, pady=4)

        add_cmd(0, 0, "/wake", lambda: (self._request_wake(), self._do_wake()))
        add_cmd(0, 1, "/sleep", self._sleep_toggle)
        add_cmd(1, 0, "/reload", self._reload_systems)
        add_cmd(1, 1, "/reboot", self._reboot_app)
        add_cmd(2, 0, "/hush", self._toggle_hush)
        add_cmd(
            2,
            1,
            "/shutdown",
            lambda: self._shutdown_app(confirm=True),
        )

        tk.Label(
            body,
            text="Tips: /wake waits for explicit launch in debug mode. /sleep can also confirm bedtime to shut down gently.",
            fg="#9fe8ff",
            bg=theme.COLORS["bg"],
            font=(theme.get_font_family(), 9),
            wraplength=360,
    ...
    def _build_mood_panel(self):
        card = tk.Frame(self.right_container, bg=theme.COLORS['bg'])
        card.grid(row=1, column=0, sticky='ew', padx=(8, 12), pady=(8, 6))
        card.grid_columnconfigure(0, weight=1)
        canvas = tk.Canvas(card, height=170, bg=theme.COLORS['bg'], highlightthickness=0)
        canvas.grid(row=0, column=0, sticky='nsew')
        inner = tk.Frame(canvas, bg=theme.COLORS['panel'], padx=6, pady=6)
        win_id = canvas.create_window((14, 14), window=inner, anchor='nw')

        def _layout(_evt=None):
            try:
                canvas.delete('hud')
                w = max(260, int(canvas.winfo_width() or 280))
                h = max(150, int(canvas.winfo_height() or 170))
                theme.round_rect(canvas, 4, 4, w - 4, h - 4, r=12, fill=theme.COLORS['panel'], outline=theme.COLORS['accent'], width=2, tags=('hud',))
                theme.round_rect(canvas, 10, 10, w - 10, h - 10, r=10, fill='#0c111b', outline='#0f2026', width=1, tags=('hud',))
                canvas.create_line(18, 30, min(w - 18, 160), 30, fill=theme.COLORS['accent_glow'], width=3, tags=('hud',))
                canvas.coords(win_id, 18, 18)
                canvas.itemconfig(win_id, width=max(120, w - 36), height=max(120, h - 36))
            except Exception:
                pass

        canvas.bind('<Configure>', _layout)
        inner.grid_columnconfigure(1, weight=1)
        tk.Label(inner, text='Feeling', fg=theme.COLORS['accent_glow'], bg='#0c111b', font=(theme.get_font_family(), 11, 'bold'), anchor='w').grid(row=0, column=0, columnspan=2, sticky='w', padx=(2, 4), pady=(0, 6))
        self.mood_card_face = tk.Label(inner, text='??', font=(theme.get_font_family(), 26, 'bold'), bg='#0c111b', fg='#ffc7ff')
        self.mood_card_face.grid(row=1, column=0, rowspan=2, padx=(2, 10), pady=(2, 2))
        self.mood_card_state = tk.Label(inner, text='Comfortable', fg=theme.COLORS['text'], bg='#0c111b', font=(theme.get_font_family(), 14, 'bold'), anchor='w')
        self.mood_card_state.grid(row=1, column=1, sticky='w', padx=(2, 6), pady=(2, 0))
        self.mood_card_tone = tk.Label(inner, text='Comfort tone -> 50%', fg=theme.COLORS['muted'], bg='#0c111b', font=(theme.get_font_family(), 10), anchor='w')
        self.mood_card_tone.grid(row=2, column=1, sticky='w', padx=(2, 6))
        self._mood_bar_width = 200
        self.mood_card_bar = tk.Canvas(inner, width=self._mood_bar_width, height=12, bg='#0c111b', highlightthickness=0)
        self.mood_card_bar.grid(row=3, column=0, columnspan=2, padx=2, pady=(6, 4), sticky='ew')
        self.mood_card_bar_fill = self.mood_card_bar.create_rectangle(0, 0, 0, 12, fill='#24c7ff', width=0)
        self.mood_card_missing = tk.Label(inner, text='All feelings accounted for.', fg=theme.COLORS['muted'], bg='#0c111b', font=(theme.get_font_family(), 9), wraplength=220, justify='left')
        self.mood_card_missing.grid(row=4, column=0, columnspan=2, sticky='w', padx=2, pady=(2, 6))
        btn_row = tk.Frame(inner, bg='#0c111b')
        btn_row.grid(row=5, column=0, columnspan=2, sticky='ew', padx=2, pady=(0, 2))
        tk.Button(btn_row, text='Emotion Catalog', command=self._open_emotion_catalog, bg=theme.COLORS['panel'], fg=theme.COLORS['text'], font=(theme.get_font_family(), 9), relief='flat', cursor='hand2', activebackground=theme.COLORS['accent'], activeforeground='#ffffff').pack(side='left', padx=(0, 6))

    def _build_status_panel_main(self):
        card = tk.Frame(self.right_container, bg=theme.COLORS['bg'])
        card.grid(row=2, column=0, sticky='ew', padx=(8, 12), pady=(0, 6))
        card.grid_columnconfigure(0, weight=1)
        canvas = tk.Canvas(card, height=150, bg=theme.COLORS['bg'], highlightthickness=0)
        canvas.grid(row=0, column=0, sticky='nsew')
        inner = tk.Frame(canvas, bg=theme.COLORS['panel'], padx=8, pady=6)
        win_id = canvas.create_window((14, 12), window=inner, anchor='nw')

        def _layout_diag(_evt=None):
            try:
                canvas.delete('hud')
                w = max(260, int(canvas.winfo_width() or 260))
                h = max(130, int(canvas.winfo_height() or 140))
                theme.round_rect(canvas, 4, 4, w - 4, h - 4, r=12, fill=theme.COLORS['panel'], outline=theme.COLORS['accent'], width=2, tags=('hud',))
                theme.round_rect(canvas, 10, 10, w - 10, h - 10, r=10, fill='#0c111b', outline='#0f2026', width=1, tags=('hud',))
                canvas.create_line(18, 26, min(w - 18, 140), 26, fill=theme.COLORS['accent2'], width=3, tags=('hud',))
                canvas.coords(win_id, 18, 14)
                canvas.itemconfig(win_id, width=max(120, w - 36), height=max(100, h - 30))
            except Exception:
                pass

        canvas.bind('<Configure>', _layout_diag)
        inner.grid_columnconfigure(1, weight=1)
        tk.Label(inner, text='Diagnostics', fg=theme.COLORS['accent_glow'], bg='#0c111b', font=(theme.get_font_family(), 11, 'bold'), anchor='w').grid(row=0, column=0, columnspan=2, sticky='w', padx=(2, 4), pady=(0, 6))
        for r, (key, label) in enumerate((('cpu', 'CPU Load'), ('ram', 'RAM'), ('uptime', 'Uptime'), ('discord', 'Discord'), ('tts', 'TTS Output'))):
            tk.Label(inner, text=f"{label}:", fg=theme.COLORS['text'], bg='#0c111b', font=(theme.get_font_family(), 10)).grid(row=r + 1, column=0, sticky='w', pady=1, padx=(2, 4))
            val = tk.Label(inner, text='--', fg='#9fe8ff', bg='#0c111b', font=(theme.get_font_family(), 10, 'bold'), anchor='w')
            val.grid(row=r + 1, column=1, sticky='w', pady=1, padx=(4, 4))
            self.health_labels[key] = val

    def _build_system_log_panel_main(self):
        card = tk.LabelFrame(
            ...
            font=(theme.get_font_family(), 11, "bold"),
            highlightthickness=1,
            highlightbackground=theme.COLORS["accent"],
        )
        card.grid(row=3, column=0, sticky="nsew", padx=(8, 12), pady=(0, 6))
        card.grid_rowconfigure(1, weight=1)
        card.grid_columnconfigure(0, weight=1)
        canvas = tk.Canvas(card, bg=theme.COLORS["bg"], highlightthickness=0)
        canvas.grid(row=1, column=0, sticky="nsew", padx=4, pady=4)
        txt = tk.Text(
            canvas,
            wrap="word",
            fg=theme.COLORS["text"],
        ...
        def _layout(_evt=None):
            try:
                canvas.delete("bg")
                w = int(canvas.winfo_width() or 260)
                ...
                canvas.itemconfig(
                    canvas._text_window, width=max(50, w - 24), height=max(50, h - 22)
                )
            except Exception:
                pass

        canvas.bind("<Configure>", _layout)
        self.system_console = txt
    ...
    def _build_keys_tab(self, parent):
        ...
        try:
            ...
            pass
        # Keep content width synced to viewport
        def _sync_mod_width(_evt=None):
            try:
                extra = vbar.winfo_width() if vbar.winfo_ismapped() else 0
                canvas.itemconfigure(
                    body_id, width=max(0, canvas.winfo_width() - extra - 4)
                )
            except Exception:
                pass

        canvas.bind("<Configure>", lambda e: (_sync_mod_width(), canvas.configure(scrollregion=canvas.bbox("all"))))
        body.bind("<Configure>", lambda e: (_sync_mod_width(), canvas.configure(scrollregion=canvas.bbox("all"))))
    ...
    def _build_modules_tab(self, parent):
        ...
        grid.columnconfigure(0, weight=1, uniform="mods_cols", minsize=260)
        grid.columnconfigure(1, weight=1, uniform="mods_cols", minsize=260)
        ...
        core.grid(row=0, column=0, sticky="nsew", pady=(0, 10), padx=(0, 6))
        core.columnconfigure(0, weight=1)
        core.rowconfigure(10, weight=1)
        ...
        src_frame = tk.Frame(core, bg=theme.COLORS["bg"])
        ...
        self._vision_sel_label = tk.Label(
            src_frame,
            text=f"Current: {self.var_vision_source.get() or 'screen'}",
            fg=theme.COLORS["muted"],
            bg=theme.COLORS["bg"],
            font=(theme.get_font_family(), 9),
            anchor="w",
        )
        self._vision_sel_label.pack(anchor="w", pady=(2, 4))

        picker_row = tk.Frame(src_frame, bg=theme.COLORS["bg"])
        picker_row.pack(fill="x", pady=(0, 4))
        self._pack_full_button(
            picker_row,
            "Select Share Source…",
            lambda: getattr(self, "_open_screen_share_picker", lambda: None)(),
        ).pack(side="left", fill="x", expand=True, padx=(0, 4))
        self._pack_full_button(
            picker_row, "List Cameras…", self._list_cameras
        ).pack(side="left", fill="x", expand=True)

        share_row = tk.Frame(core, bg=theme.COLORS["bg"])
        share_row.pack(fill="x", pady=(4, 2))
        self._pack_full_button(
            share_row,
            "Share Screen",
            lambda: self._set_vision_source_and_enable("screen"),
        ).pack(side="left", expand=True, fill="x", padx=(0, 4))
        self._pack_full_button(
            share_row,
            "Share Camera",
            lambda: self._set_vision_source_and_enable("camera"),
        ).pack(side="left", expand=True, fill="x")

        tk.Label(
            core,
            text="Discord/audio routing lives on the Audio tab.",
            bg=theme.COLORS["bg"],
            fg="#7fa9c2",
            font=(theme.get_font_family(), 9),
        ...
        modules.grid(row=0, column=1, sticky="nsew", pady=(0, 10), padx=(6, 0))
        modules.columnconfigure((0, 1), weight=1, uniform="mod_btn_cols")
        ...
        def add_module_btn(r: int, c: int, label: str, cmd):
            btn = self._make_hud_button(
                modules, label, cmd, width=self._button_full_width()
            )
            btn.grid(row=r, column=c, sticky="ew", padx=4, pady=4)

        add_module_btn(2, 0, "Start Session", self._start_therapy_session)
        add_module_btn(2, 1, "Stop Session", self._stop_therapy_session)
        add_module_btn(3, 0, "Story Time", self._start_story_time)
        add_module_btn(3, 1, "Browse Library", self._browse_story_files)
        add_module_btn(4, 0, "Import ChatGPT Link", self._import_story_link)
        add_module_btn(4, 1, "Import Story File…", self._import_story_file)
        add_module_btn(5, 0, "Quiet (Hush)", self._toggle_hush)
        add_module_btn(5, 1, "Reflect…", self._prompt_reflection)
        add_module_btn(
            6, 0, "Ground", lambda: self._owner_command("you're grounded")
        )
        add_module_btn(
            6, 1, "Release", lambda: self._owner_command("you're free now")
        )
        add_module_btn(7, 0, "Discord Transcript", self._open_discord_transcript)
    ...
    def _build_settings_tab(self, parent):
        ...
        card = tk.Frame(
            ...
            bd=1,
            highlightthickness=1,
            highlightbackground="#1ac6ff",
        )
        card.pack(fill="both", expand=True, padx=8, pady=8)

        header = tk.Frame(card, bg=theme.COLORS["bg"])
        ...
        tk.Label(
            ...
            fg="#7fe5ff",
            bg=theme.COLORS["bg"],
            font=(theme.get_font_family(), 15, "bold"),
        ).pack(side="left")
        tk.Label(
            header,
            text="System profile, cognition, and routing.",
            fg=theme.COLORS["muted"],
            bg=theme.COLORS["bg"],
            font=(theme.get_font_family(), 10),
        ).pack(side="left", padx=(10, 0))

        # Power icon button (safe shutdown)
        power = tk.Canvas(
            header, width=32, height=32, bg=theme.COLORS["bg"], highlightthickness=0
        )
        power.pack(side="right", padx=(0, 4))
        p_center = 16
        power.create_oval(4, 4, 28, 28, outline="#2cd9ff", width=2)
        power.create_line(p_center, 8, p_center, 18, fill="#2cd9ff", width=3)
        power.create_arc(6, 6, 26, 26, start=30, extent=300, style="arc", outline="#2cd9ff", width=2)
        power.bind("<Button-1>", lambda _e: self._shutdown_app(confirm=True))
        power.bind("<Enter>", lambda _e: power.itemconfig(1, outline="#6fffe0"))
        power.bind("<Leave>", lambda _e: power.itemconfig(1, outline="#2cd9ff"))

        divider = tk.Frame(card, bg="#12384a", height=2)
        divider.pack(fill="x", padx=10, pady=(0, 8))

        content = tk.Frame(card, bg=theme.COLORS["bg"])
        content.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        content.columnconfigure(0, weight=1, uniform="settings_cols", minsize=320)
        content.columnconfigure(1, weight=1, uniform="settings_cols", minsize=320)

        # Helpers
        def add_toggle(container, text, var, cmd):
            cb = tk.Checkbutton(
                container,
                text=text,
                variable=var,
        ...
        left = tk.Frame(content, bg=theme.COLORS["bg"])
        right = tk.Frame(content, bg=theme.COLORS["bg"])
        ...
        try:
            ...
            opt = tk.OptionMenu(
                cog, self.var_mode, *modes, command=lambda m: self._set_mode(m)
            )
            opt.configure(
                bg="#1a1c22",
                fg=theme.COLORS["text"],
            ...
            opt.pack(anchor="w", padx=6, pady=(0, 6))

            chat_models = ["gpt-5", "gpt-4.1", "gpt-4o", "gpt-4o-mini"]
            ...
            opt_model.pack(anchor="w", padx=6, pady=(0, 6))
            self._make_hud_button(cog, "Test OpenAI", self._test_openai).pack(
                anchor="w", padx=6, pady=(2, 6)
            )
        except Exception:
            tk.Label(
                cog,
                text="Cognition modules unavailable.",
                fg="#ff8888",
                bg=theme.COLORS["bg"],
                font=(theme.get_font_family(), 10),
        ...
        try:
            ...
            add_toggle(priv, "Private Mode (no memory save)", self.var_private, _toggle_private)
        except Exception:
            tk.Label(
                priv,
                text="(Privacy toggle unavailable)",
                fg="#888888",
                bg=theme.COLORS["bg"],
                font=(theme.get_font_family(), 9),
            ).pack(anchor="w", padx=6, pady=4)
        ...
        tk.Label(
            priv,
            text="VAD & voice controls live on the Audio tab.",
            fg=theme.COLORS["muted"],
            bg=theme.COLORS["bg"],
            font=(theme.get_font_family(), 9),
        ).pack(anchor="w", padx=6, pady=(4, 2))
        ...
        style.pack(fill="x", pady=(0, 10))
        self._make_hud_button(
            style, "Toggle Custom Skin Preview", self._toggle_skin_preview
        ).pack(anchor="w", padx=6, pady=(4, 6))

        # --- Routing / automation block (right) ---
        auto = tk.LabelFrame(
            right,
        ...
        add_toggle(auto, "Topmost on wake (UI)", self.var_topmost, self._toggle_topmost)
        add_toggle(auto, "Proactive chatter", self.var_proactive, self._toggle_proactive)
        add_toggle(auto, "Tasks engine", self.var_tasks, self._toggle_tasks)
        add_toggle(auto, "Awareness core", self.var_awareness, self._toggle_awareness)

        # Equalizer look-alike footer
        eq = tk.Canvas(card, height=90, bg=theme.COLORS["bg"], highlightthickness=0)
        eq.pack(fill="x", padx=10, pady=(0, 6))
        try:
            width = 680
            bars = 38
            spacing = width / bars
            ...
            for i in range(bars):
                h = 20 + (i * 7) % 50
                x0 = 14 + int(i * spacing)
                x1 = x0 + int(spacing * 0.55)
                y0 = 80 - h
                eq.create_rectangle(x0, y0, x1, 80, fill=accent, width=0)
        except Exception:
            pass

        # VoiceMeeter block (spans both columns)
        if _SHOW_VOICEMEETER_UI:
            vm_card = tk.LabelFrame(
                content,
            ...
            vm_card.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(4, 4))

            vm_row = tk.Frame(vm_card, bg=theme.COLORS["bg"])
            vm_row.pack(anchor="w", fill="x", pady=(0, 4), padx=6)
            tk.Label(
                vm_row,
                text="Preset",
                bg=theme.COLORS["bg"],
            ...
            def _entry(parent, label, var):
                frame = tk.Frame(parent, bg=theme.COLORS["bg"])
                frame.pack(anchor="w", fill="x", pady=2, padx=6)
                tk.Label(
                    frame,
                    text=label,
                    bg=theme.COLORS["bg"],
            ...
            _entry(vm_card, "A1 Device", self.var_vm_a1)
            _entry(vm_card, "A2 Device", self.var_vm_a2)
            _entry(vm_card, "Desktop Hint", self.var_vm_desktop)
            _entry(vm_card, "TTS Hint", self.var_vm_tts)

            vm_btn = tk.Frame(vm_card, bg=theme.COLORS["bg"])
            vm_btn.pack(anchor="w", pady=(2, 8), padx=6)
            ...
            try:
                ...
                scene_defs = []
            if scene_defs:
                scene_frame = tk.Frame(vm_card, bg=theme.COLORS["bg"])
                scene_frame.pack(anchor="w", fill="x", pady=(4, 2), padx=6)
                tk.Label(
                    scene_frame,
                    text="Preset Scene",
                    bg=theme.COLORS["bg"],
    ...
    def _program_usb_ui(self):
        try:
            ...
            def _enum():
                tree.delete(*tree.get_children())
                for item in self._enum_drives_with_labels():
                    try:
                        root, label = item[0], item[1] if len(item) > 1 else ""
                        tree.insert("", "end", values=(root, label))
                    except Exception as exc:
                        self.safe_log(f"USB UI error: {exc}", "#ff5555")
    ...
    def _refresh_gaming_status(self, reschedule: bool = True):
        ...
        try:
            ...
            events = []
        if getattr(self, "_gaming_events_box", None) and self._gaming_events_box.winfo_exists():
            lines = []
            for evt in events[-8:]:
    ...
    def _apply_vision_input(self):
        ...
        try:
            ...
            pass
        try:
            if getattr(self, "_vision_sel_label", None):
                self._vision_sel_label.config(
                    text=f"Current: {mode} ({name or f'index {index}'})"
                )
        except Exception:
            pass

    def _set_vision_source_and_enable(self, mode: str):
        try:
            self.var_vision_source.set(mode)
            self._apply_vision_input()
            if not self.var_vision.get():
                self.var_vision.set(True)
                self._toggle_vision()
        except Exception:
            self.safe_log(f"Vision setup failed for {mode}.", "#ff5555")

    def _open_screen_share_picker(self):
        """Minimal screen/window picker to emulate Discord-like choice."""
        win = tk.Toplevel(self.root)
        win.title("Select screen/window to share")
        win.configure(bg=theme.COLORS["bg"])
        win.grab_set()
        try:
            win.resizable(False, False)
        ...
        screens_tab = tk.Frame(tabs, bg=theme.COLORS["bg"])
        windows_tab = tk.Frame(tabs, bg=theme.COLORS["bg"])
        tabs.add(screens_tab, text="Screens")
        tabs.add(windows_tab, text="Windows")

        screen_list = tk.Listbox(
            screens_tab,
            height=4,
            bg="#0f131a",
            fg="#cde6ff",
            selectbackground="#1f4f68",
            selectforeground="#ffffff",
            activestyle="none",
        )
        screen_list.pack(fill="both", expand=True, padx=6, pady=6)
        # Basic screen candidates; primary + a second placeholder
        screen_list.insert("end", "Primary Monitor (All)")
        screen_list.insert("end", "Monitor 1")
        screen_list.insert("end", "Monitor 2")
        screen_list.selection_set(0)
        ...
        win_list.pack(fill="both", expand=True, padx=6, pady=6)
        try:
            titles = []
            try:
                ...
                titles = []
            if not titles:
                win_list.insert("end", "(No windows detected — using full screen.)")
            else:
                for t in titles:
                    win_list.insert("end", t)
        except Exception:
            win_list.insert("end", "(Window enumeration unavailable.)")

        choice = {"mode": "screen", "name": "", "index": 0}
        ...
        def _on_confirm():
            ...
            if tab == 0:
                ...
                # Map: 0=all, 1=monitor1, 2=monitor2
                choice["mode"] = "screen"
                choice["index"] = max(0, idx - 0)
                choice["name"] = f"monitor_{idx}"
            else:
                sel = win_list.curselection()
                if sel:
                    title = win_list.get(sel[0])
                    choice["mode"] = "screen"
                    choice["name"] = f"window:{title}"
                    choice["index"] = 0
                else:
                    choice["mode"] = "screen"
                    choice["name"] = "window:any"
                    choice["index"] = 0
            win.destroy()

        btns = tk.Frame(win, bg=theme.COLORS["bg"])
        btns.pack(fill="x", padx=8, pady=(0, 10))
        self._make_hud_button(btns, "Cancel", win.destroy).pack(
            side="right", padx=4
        )
        self._make_hud_button(btns, "Share", _on_confirm).pack(
            side="right", padx=4
        )
        ...
        if choice["mode"]:
            try:
                self.var_camera_name.set(choice["name"])
                self.var_camera_index.set(str(choice["index"]))
                self.var_vision_source.set(choice["mode"])
                self._set_vision_source_and_enable(choice["mode"])
            except Exception:
                pass
    ...
    def _open_channel_settings(self):
        ...
        def _new_channel():
            try:
                self._channel_ctx_fields["id"].set("")
                self._channel_ctx_fields["label"].set("")
                self._channel_ctx_fields["instructions"].set("")
                self._channel_ctx_fields["relationship"].set("")
                self._channel_ctx_fields["allow"].set(True)
        ...
        self._make_hud_button(btn_row, "New", _new_channel).pack(
            side="left", padx=(0, 6)
        )
    ...
    def _confirm_and_sleep(self):
        if not getattr(self, "_awake", False):
            self.log("Systems are already idle — no need to say goodnight.", "#ffaa00")
            return
        try:
    ...
    def _update_wake_strip(self):
        try:
            if getattr(self, "_wake_strip", None) is None:
                return
            # Keep wake controls visible so the user always has a quick launch control.
    ...
    def _owner_command(self, text: str):
        ...
        def _worker():
            reply = self._handle_input(msg)
            if reply:
                try:
                    self.safe_log(reply, "#a6f3c6")
                except Exception:
                    pass
        threading.Thread(target=_worker, daemon=True).start()

    def _handle_enter(self):
        # Backward compat wrapper for older bindings
        self._on_enter(None)
    ...
    def _handle_input(self, msg):
        ...
        thread = threading.Thread(target=compute_and_update, daemon=True)
        thread.start()
        thread.join(0)
        return ""
    ...
    def _update_cognition_badge(self):
        try:
            ...
            # Update routing label (Voicemeeter removed)
            try:
                self.route_label.config(
                    text=f"Routing: {getattr(self,'_routing_mode','Custom').title()}"
                )
            except Exception:
                pass
        except Exception:
    ...
    def _tick_hearing(self):
        ...
        try:
            now = time.time()
            # Keep bars warm if listening modes are enabled (even without recent samples)
            try:
                if bool(self.var_listen.get()):
                    self._last_hear_t_mic = now
                if bool(self.var_desktop_listen.get()):
                    self._last_hear_t_desktop = now
            except Exception:
                pass
    ...
    def _do_wake(self, dev_override: bool = False):
        try:
            ...
            try:
                ...
                ok = True
            # If verification failed but no password was provided, allow boot to proceed for local testing.
            if not ok and not pw:
                ok = True
            if not ok:
                try:
                    self._wake_msg.config(
            ...
            try:
                ...
                pass
            # Run boot in background to avoid UI freeze
            self._start_boot_sequence(boot, _cl)
        except Exception as e:
            self.safe_log(f"Wake failed: {e}", "#ff5555")
            self._booting = False

    def _start_boot_sequence(self, boot, _cl):
        def _worker():
            try:
                steps = getattr(boot, "boot_steps", None)
                if callable(steps):
                    for name, fn in steps():
                        ...
                        try:
                            ...
                            self.safe_log(
                                f"✅ {name} online ({elapsed:.2f}s)", "#a6f3c6"
                            )
                        except Exception as exc:
                            self.safe_log(
                                f"❌ {name} failed: {exc}", "#ff5555"
                            )
                            return  # stop remaining steps
                    self._finish_wake_safe(_cl)
                    return
                # Fallback to old boot_all capture
                buf = io.StringIO()
                ...
                self.safe_log(f"Boot encountered an error: {exc}", "#ff5555")

        threading.Thread(target=_worker, daemon=True).start()

    def _finish_wake_safe(self, _cl):
        try:
            _cl.start_background()
        except Exception as exc:
            self.safe_log(f"Background start failed: {exc}", "#ff5555")
            self._booting = False
            return
        try:
    ...
    def _auto_start_titanfall(self):
        """Best-effort one-click start for Titanfall monitor/autopilot."""
        try:
            try:
                gaming_bridge.configure_titanfall(enabled=False, auto_voice=True)
            except Exception:
                pass
            try:
                tf_path = os.getenv("TITANFALL2_PATH", r"C:\Program Files (x86)\Steam\steamapps\common\Titanfall2")
                launcher = os.path.join(tf_path, "NorthstarLauncher.exe")
                if os.path.isfile(launcher):
                    subprocess.Popen([launcher], cwd=tf_path)
                    self.safe_log("Northstar launcher started.", "#ffae6b")
            except Exception as exc:
                self.safe_log(f"Northstar launch skipped: {exc}", "#ffaa00")
            try:
                if hasattr(gaming_bridge, "start_monitor"):
                    gaming_bridge.start_monitor()
            except Exception:
                pass
            try:
                self.var_titanfall_enabled.set(True)
            except Exception:
                pass
            self._open_titanfall_window()
            observe_ms = 90000  # observe 90s before takeover
            self.safe_log(
                "Titanfall auto-start: monitoring now, will take over autopilot after ~90s.",
                "#ffae6b",
            )
            try:
                self.root.after(observe_ms, lambda: self._enable_titanfall_autopilot(True))
            except Exception:
                self._enable_titanfall_autopilot(True)
        except Exception as exc:
            self.safe_log(f"Titanfall auto-start error: {exc}", "#ff5555")

    def _enable_titanfall_autopilot(self, flag: bool):
        try:
            gaming_bridge.configure_titanfall(enabled=flag)
            self.var_titanfall_enabled.set(flag)
            status = gaming_bridge.get_titanfall_status()
            note = "Autopilot engaged after observation." if flag else "Autopilot disabled."
            self._log_titanfall_notes(note, status)
            self.safe_log(
                f"Titanfall autopilot {'ENABLED' if flag else 'disabled'}.",
                "#55ff88" if flag else "#ffaa00",
            )
        except Exception as exc:
            self.safe_log(f"Titanfall autopilot toggle failed: {exc}", "#ff5555")

    def _log_titanfall_notes(self, note: str, status: dict | None = None):
        try:
            logs_dir = Path(__file__).resolve().parents[1] / "logs"
            logs_dir.mkdir(exist_ok=True)
            path = logs_dir / "titanfall_notes.log"
            ts = datetime.utcnow().isoformat() + "Z"
            status = status or {}
            titan = status.get("titan") or {}
            mission = status.get("match") or {}
            with path.open("a", encoding="utf-8") as f:
                f.write(
                    f"{ts} | {note} | HP:{titan.get('health')} SH:{titan.get('shield')} "
                    f"Core:{titan.get('core_ready')} Map:{mission.get('map')} Mode:{mission.get('mode')} "
