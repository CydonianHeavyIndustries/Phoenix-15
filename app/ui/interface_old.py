...
class BjorgsunUI:
    ...
    def _build_modules_tab(self, parent):
        ...
        tk.OptionMenu(row, self.var_vision_source, "screen", "camera").pack(side="left")
        tk.Entry(row, textvariable=self.var_camera_name, width=18).pack(
            side="left", padx=(6, 4)
        )
        tk.Entry(row, textvariable=self.var_camera_index, width=4).pack(side="left")
        btns = tk.Frame(src_frame, bg=theme.COLORS["bg"])
        btns.pack(fill="x", pady=(2, 0))
        self._make_hud_button(
            btns, "Apply Vision Input", self._apply_vision_input
        ).pack(side="left", padx=(0, 6))
        self._make_hud_button(btns, "List Cameras…", self._list_cameras).pack(
            side="left"
        )
        ...
        try:
            ...
            self._make_hud_button(col, "Tasks…", self._open_tasks).pack(
                anchor="w", pady=(0, 6)
            )
            self._make_hud_button(col, "Sleep", self._sleep_toggle).pack(
                anchor="w", pady=(0, 6)
            )
            self._make_hud_button(col, "I'm Awake!", self._awake_brief).pack(
                anchor="w", pady=(0, 6)
            )
            self._make_hud_button(col, "Overlay", self._toggle_overlay).pack(
                anchor="w", pady=(0, 6)
            )
            ...
            self._make_hud_button(col, "Reflections…", self._open_reflections).pack(
                anchor="w", pady=(0, 6)
            )
            ...
            self._make_hud_button(col, "Self Test", self._run_selftest).pack(
                anchor="w", pady=(0, 6)
            )
            self._make_hud_button(col, "Dashboard…", self._open_owner_dashboard).pack(
                anchor="w", pady=(0, 6)
            )
            self._make_hud_button(
        ...
        row1 = tk.Frame(owner_card, bg=theme.COLORS["bg"])
        row1.pack(fill="x", pady=(4, 2))
        self._make_hud_button(
            row1, "Ground", lambda: self._owner_command("you're grounded")
        ).pack(side="left", padx=2)
        self._make_hud_button(
            row1, "Release", lambda: self._owner_command("you're free now")
        ).pack(side="left", padx=2)
        self._make_hud_button(row1, "Reflect…", self._prompt_reflection).pack(
            side="left", padx=2
        )
        row2 = tk.Frame(owner_card, bg=theme.COLORS["bg"])
        row2.pack(fill="x", pady=(2, 6))
        self._make_hud_button(row2, "Quiet (Hush)", self._toggle_hush).pack(
            side="left", padx=2
        )
        self._make_hud_button(
            row2, "Discord Transcript", self._open_discord_transcript
        ).pack(side="left", padx=2)
        ...
        try:
            ...
            btn_row = tk.Frame(tf, bg=theme.COLORS["bg"])
            btn_row.pack(anchor="w", pady=(0, 4))
            self._make_hud_button(
                btn_row, "Start Session", self._start_therapy_session
            ).pack(side="left", padx=(0, 6))
            self._make_hud_button(
                btn_row, "Stop Session", self._stop_therapy_session
            ).pack(side="left")
        except Exception:
            pass
        ...
        try:
            ...
            link_row = tk.Frame(sf, bg=theme.COLORS["bg"])
            link_row.pack(fill="x", pady=(0, 4))
            tk.Entry(link_row, textvariable=self.var_story_link, width=32).pack(
                side="left", fill="x", expand=True, padx=(0, 6)
            )
            self._make_hud_button(
                link_row, "Import ChatGPT Link", self._import_story_link
            ).pack(side="left")
            btn_row = tk.Frame(sf, bg=theme.COLORS["bg"])
            btn_row.pack(anchor="w", pady=(0, 4))
            self._make_hud_button(
                btn_row, "Import Story File…", self._import_story_file
            ).pack(side="left", padx=(0, 6))
            self._make_hud_button(btn_row, "Story Time", self._start_story_time).pack(
                side="left", padx=(0, 6)
            )
            self._make_hud_button(
                btn_row, "Browse Library", self._browse_story_files
            ).pack(side="left")
        except Exception:
            pass
    ...
    def _build_discord_tools(self, container):
        try:
            ...
            btn_row = tk.Frame(frame, bg=theme.COLORS["bg"])
            btn_row.pack(fill="x", pady=(0, 4))
            self._make_hud_button(
                btn_row, "Refresh Status", self._refresh_discord_status
            ).pack(side="left", padx=(0, 6))
            self._make_hud_button(
                btn_row, "Restart Bridge", self._restart_discord_bridge
            ).pack(side="left", padx=(0, 6))
            ...
            advanced = tk.Frame(frame, bg=theme.COLORS["bg"])
            advanced.pack(anchor="w", pady=(0, 4))
            self._make_hud_button(
                advanced, "Channel Settings…", self._open_channel_settings
            ).pack(side="left", padx=(0, 6))
            self._make_hud_button(
                advanced, "Family IDs…", self._open_family_editor
            ).pack(side="left")
    ...
    def _build_gaming_panel(self, parent):
        try:
            ...
            ctrl = tk.Frame(card, bg=theme.COLORS["bg"])
            ctrl.pack(fill="x", pady=(0, 6), padx=6)
            tk.Checkbutton(
                ctrl,
            ...
            key_row = tk.Frame(card, bg=theme.COLORS["bg"])
            ...
            btns = tk.Frame(ctrl, bg=theme.COLORS["bg"])
            btns.pack(side="right")
            self._make_hud_button(btns, "Open Stream Page", self._open_stream_url).pack(
                side="right", padx=(6, 0)
            )
            self._make_hud_button(
                btns,
            ...
            tk.Entry(
                row_tf_voice, textvariable=self.var_titanfall_voice_channel, width=18
            ).pack(side="left", padx=4)
            self._make_hud_button(
                row_tf_voice, "Save", self._save_titanfall_voice_channel
            ).pack(side="left")
            row_tf_brief = tk.Frame(titan_card, bg=theme.COLORS["bg"])
            ...
            tk.Entry(
                row_tf_brief, textvariable=self.var_titanfall_briefing_channel, width=18
            ).pack(side="left", padx=4)
            self._make_hud_button(
                row_tf_brief, "Save", self._save_titanfall_briefing_channel
            ).pack(side="left")
            row_tf_report = tk.Frame(titan_card, bg=theme.COLORS["bg"])
            ...
            tk.Entry(
                row_tf_report, textvariable=self.var_titanfall_report_channel, width=18
            ).pack(side="left", padx=4)
            self._make_hud_button(
                row_tf_report, "Save", self._save_titanfall_report_channel
            ).pack(side="left")
            row_tf_idle = tk.Frame(titan_card, bg=theme.COLORS["bg"])
            ...
            tk.Entry(
                row_tf_idle, textvariable=self.var_titanfall_idle_channel, width=18
            ).pack(side="left", padx=4)
            self._make_hud_button(
                row_tf_idle, "Save", self._save_titanfall_idle_channel
            ).pack(side="left")
            row_tf_call = tk.Frame(titan_card, bg=theme.COLORS["bg"])
            row_tf_call.pack(fill="x", padx=6, pady=(2, 4))
            tk.Label(
                row_tf_call,
                text="Callsign:",
                bg=theme.COLORS["bg"],
            ...
            tk.Entry(
                row_tf_call, textvariable=self.var_titanfall_callsign, width=18
            ).pack(side="left", padx=4)
            self._make_hud_button(
                row_tf_call, "Save", self._save_titanfall_callsign
            ).pack(side="left")
            tk.Label(
                titan_card,
    ...
    def _make_hud_button(self, parent, text: str, command, width: int | None = None):
        ...
        def draw(state="normal"):
            ...
            fill = (
                "#20313b"
                if state == "normal"
                else ("#2a4656" if state == "hover" else "#16303a")
            )
